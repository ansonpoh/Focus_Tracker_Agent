...
def build_email_html(report_text: str, pdf_filename: str) -> str:
    elements = parse_report_elements(report_text)
    blocks = [
        "<!doctype html>",
        "<html>",
    ...
    for element in elements:
        ...
        if element_type == "title":
            blocks.append(f"<h1 style=\"margin:0 0 18px;font-size:28px;line-height:1.2;color:#111827;\">{html.escape(str(element['text']))}</h1>")
        elif element_type == "heading":
            blocks.append(f"<h2 style=\"margin:26px 0 10px;padding-bottom:8px;border-bottom:1px solid #e5e7eb;font-size:18px;line-height:1.3;color:#111827;\">{html.escape(str(element['text']))}</h2>")
        elif element_type == "paragraph":
            blocks.append(f"<p style=\"margin:10px 0;font-size:14px;line-height:1.65;color:#1f2937;\">{htmlize_inline_text(str(element['text']))}</p>")
        elif element_type == "bullet":
            blocks.append(f"<p style=\"margin:8px 0 8px 0;padding-left:14px;font-size:14px;line-height:1.6;color:#1f2937;\">&bull; {htmlize_inline_text(str(element['text']))}</p>")
        elif element_type == "table":
            rows = element["rows"]
            if isinstance(rows, list) and rows:
                blocks.append(build_html_table(rows))
...
def render_report_pdf(report_path: Path, report_text: str) -> Path:
    ...
    pdf_path.write_bytes(build_styled_pdf(parse_report_elements(report_text)))
    return pdf_path
...
def htmlize_inline_text(value: str) -> str:
...
def parse_report_elements(report_text: str) -> list[dict[str, object]]:
...
def build_html_table(rows: list[list[str]]) -> str:
    ...
    for cell in header:
        parts.append(f"<th style=\"text-align:left;padding:10px 12px;background:#f3f4f6;border:1px solid #d1d5db;font-weight:600;\">{htmlize_inline_text(cell)}</th>")
    parts.append("</tr></thead><tbody>")
    for row in body:
        parts.append("<tr>")
        for cell in row:
            parts.append(f"<td style=\"padding:10px 12px;border:1px solid #e5e7eb;vertical-align:top;\">{htmlize_inline_text(cell)}</td>")
        parts.append("</tr>")
    parts.append("</tbody></table>")
    return "".join(parts)


def text_wrap_for_width(text: str, width_points: float, font_size: int) -> list[str]:
    approx_chars = max(12, int(width_points / max(font_size * 0.56, 1)))
    return wrap_text(text, approx_chars)


def build_styled_pdf(elements: list[dict[str, object]]) -> bytes:
    page_height = 792
    page_width = 612
    left_margin = 48
    right_margin = 48
    ...
    def write_text(text: str, *, x: float, y: float, font: str, size: int) -> None:
        pages[-1].append("\n".join(["BT", f"/{font} {size} Tf", f"1 0 0 1 {x:.2f} {y:.2f} Tm", f"({escape_pdf_text(text)}) Tj", "ET"]))
    ...
    for element in elements:
        ...
        if element_type == "paragraph":
            wrapped = text_wrap_for_width(str(element["text"]), content_width, 10)
        ...
        if element_type == "bullet":
            wrapped = text_wrap_for_width(str(element["text"]), content_width - 18, 10)
    ...
    for page_index, (page_commands, (content_id, page_id)) in enumerate(zip(pages, page_pairs, strict=True), start=1):
        footer = ["BT", "/F1 9 Tf", f"1 0 0 1 {left_margin:.2f} 24.00 Tm", f"(Page {page_index} of {len(pages)}) Tj", "ET"]
        content_stream = "\n".join(page_commands + footer).encode("ascii")
        objects[content_id] = f"<< /Length {len(content_stream)} >>\nstream\n".encode("ascii") + content_stream + b"\nendstream"
        objects[page_id] = (
...
def render_table(
    *,
    ...
    def cell_lines(text: str, width: float) -> list[str]:
        return text_wrap_for_width(text, width - 10, 9)
    ...
    def draw_row(row: list[str], *, is_header: bool) -> None:
        ...
        for cell, width in zip(row, widths, strict=True):
            ...
            for line in cell_lines(cell, width):
                pages[-1].append("\n".join(["BT", f"/{font} 9 Tf", f"1 0 0 1 {x + 5:.2f} {text_y:.2f} Tm", f"({escape_pdf_text(line)}) Tj", "ET"]))
                text_y -= 12
            x += width
        current_y -= height
